...
# Check if payment detected (watcher is broken, so we manually mark as paid)
echo "⚠️  Watcher Lambda is currently broken. Manually marking as paid..."
...
echo ""
echo "⏳ Waiting for sweep (30 seconds)..."
sleep 30

# Check final status
STATUS=$(aws dynamodb get-item --table-name SuiInvoices \
  --key "{\"invoice_id\": {\"S\": \"$INVOICE_ID\"}}" \
  --query 'Item.status.S' --output text --region us-east-1)
...
if [ "$STATUS" == "swept" ]; then
  ...
  TX_DIGEST=$(aws dynamodb get-item --table-name SuiInvoices \
    --key "{\"invoice_id\": {\"S\": \"$INVOICE_ID\"}}" \
    --query 'Item.tx_digest.S' --output text --region us-east-1)
