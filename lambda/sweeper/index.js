...
async function ensureSufficientGas(userWallet, hotWallet, estimatedFee, gasPrice, invoiceId, currency) {
    let ethBalance = await provider.getBalance(userWallet.address);

    if (ethBalance < estimatedFee) {
        console.log(`Insufficient ETH to cover gas for ${currency} invoice ${invoiceId}: ${ethBalance} < ${estimatedFee}`);
...
exports.handler = async (event) => {
    ...
    const mnemonicSecret = await secretsManager.getSecretValue({ SecretId: "hd-wallet-mnemonic" }).promise();
    ...
    for (const record of event.Records) {
        ...
        try {
            console.log(`Processing invoice ${invoiceId} | Address: ${userWallet.address} | Currency: ${currency}`);
            ...
            if (currency === "ETH") {
                ...
                let ethBalance = await ensureSufficientGas(userWallet, hotWallet, estimatedFee, gasPrice, invoiceId, "ETH");
                ...
                const txRequest = await token.getFunction("transfer").populateTransaction(DEPOSIT_WALLET_ADDRESS, tokenBalance);
                estimatedGas = await provider.estimateGas({ ...txRequest, from: userWallet.address });
                estimatedGas = addGasBuffer(estimatedGas);
                estimatedFee = estimatedGas * gasPrice;

                await ensureSufficientGas(userWallet, hotWallet, estimatedFee, gasPrice, invoiceId, "ERC-20");

                const decimals = await token.decimals();
                console.log(`Sweeping ${ethers.formatUnits(tokenBalance, decimals)} ${tokenSymbol} to treasury...`);
