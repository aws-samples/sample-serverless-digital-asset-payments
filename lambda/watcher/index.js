...
exports.handler = async () => {
        ...
        try {
            if (currency === "ETH") {
                ...
                if (balance >= required) {
                    ...
                    throw new Error(`Insufficient ETH: required ${amount}, got ${ethers.formatEther(balance)}`);
                }
                ...
                if (balance >= required) {
                    ...
                    throw new Error(`Insufficient ${tokenSymbol}: required ${amount}, got ${ethers.formatUnits(balance, decimals)}`);
                }
            }
    ...
    console.log(`Processing completed: ${processed.length} invoices processed successfully, ${failed.length} invoices failed.`);
    return {
        status: "completed",
        processed,
        failed
